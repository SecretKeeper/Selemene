...
class SocketBroadcastListener @Inject constructor(
    ...
    fun onDestroyMessagesArrayListener(): Socket.OnEventResponseListener =
        object : Socket.OnEventResponseListener() {
            override fun onMessage(event: String?, data: String?) {

                val arrayMessageAdapter: JsonAdapter<DestroyMessageIds> =
                    moshi.adapter(DestroyMessageIds::class.java)

                Log.e("SocketBroadCastListener", "DestroyArrayMessageListener: $data")
                ...
                if (data != null) {
                    arrayMessageAdapter.fromJson(data)?.let { it ->
                        ...
                        if (!isAppRunning(application, "net.teamof.whisper")) {
                            CoroutineScope(Dispatchers.IO).launch {
                                messageRepository.delete(it.message_ids)
                            }
                        } else {
                            application.sendBroadcast(
                                Intent("WhisperLocalMessageCommunication").putExtra(
